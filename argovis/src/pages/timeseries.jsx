...
class Timeseries extends React.Component {
    ...
    gridRasterfy(state){
        let gridsize = 0.25 // should be part of state, tbd
        ...
        if(state.data.hasOwnProperty('code') || state.data[0].hasOwnProperty('code')){
            ...
            let points = state.data[0].map(point => {return(
                <Rectangle 
                    key={Math.random()} 
                    bounds={[[point.geolocation.coordinates[1]-gridsize/2, helpers.mutateLongitude(point.geolocation.coordinates[0], parseFloat(state.centerlon))-gridsize/2],[point.geolocation.coordinates[1]+gridsize/2, helpers.mutateLongitude(point.geolocation.coordinates[0], parseFloat(state.centerlon))+gridsize/2]]} 
                    pathOptions={{ 
                        fillOpacity: 0.5, 
                        weight: 0, 
